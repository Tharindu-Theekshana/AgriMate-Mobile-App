...
import { Body, Button, Card, SectionHeader } from '@/components/ui';
import { useAuth } from '@/context/AuthContext';
import { useColors } from '@/context/ThemeContext';
import { useToast } from '@/context/ToastContext';
import { listCrops } from '@/data/crops';
import { listFarms } from '@/data/farms';
import { cacheScan } from '@/data/scans';
...
export default function ScanScreen() {
  ...
  async function analyze() {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        // Map local selections to server ids (offline-created farms sync first).
        ...
        await cacheScan(scan); // available offline afterwards
      } else {
        // Guest: AI only, not persisted server-side.
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxl }}>
      ...
      {/* Farm / crop selectors (account only) */}
        ...
        <>
          ...
          {crops.length > 0 && (
            <>
              ...
              <Chips
                colors={colors}
                items={crops.map((c) => ({ id: c.id, label: `${c.cropType}${c.growthStage ? ` · ${c.growthStage}` : ''}` }))}
                selected={cropId}
