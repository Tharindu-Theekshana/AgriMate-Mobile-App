...
import { createCrop, suggestHarvestDate, updateCrop } from '@/data/crops';
import type { LocalCrop, Season } from '@/data/types';
import { ChipSelect, DateField } from '@/components/form';
import { Button, Screen, TextField, Title } from '@/components/ui';
import { useColors } from '@/context/ThemeContext';
import { useToast } from '@/context/ToastContext';
import { spacing } from '@/theme/theme';
import { PADDY_STAGES } from '@/utils/crop';
...
export function CropFormModal({
  ...
  const [stage, setStage] = useState<string | null>(initial?.growthStage ?? null);
  ...
  async function save() {
    ...
    const input = {
      ...
      expectedHarvestDate: harvestDate || (plantingDate ? suggestHarvestDate(plantingDate) ?? undefined : undefined),
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <Screen scroll>
        ...
        <DateField
          ...
          value={harvestDate || (plantingDate ? suggestHarvestDate(plantingDate) ?? '' : '')}
        ...
        <ChipSelect
          ...
          allowClear
          onChange={setStage}
          options={PADDY_STAGES.map((s) => ({ value: t(`crop.stages.${s.key}`), label: t(`crop.stages.${s.key}`) }))}
        />
