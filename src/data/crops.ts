...
import type { CropStatus, LocalCrop, Season } from '@/data/types';
...
/** ~105-day paddy cycle: suggest a harvest date from the planting date. */
export function suggestHarvestDate(plantingDate?: string | null): string | null {
  if (!plantingDate) return null;
  const d = new Date(plantingDate);
  if (Number.isNaN(d.getTime())) return null;
  d.setDate(d.getDate() + 105);
...
export async function createCrop(farmId: string, input: CropInput): Promise<LocalCrop> {
  const id = uuid();
  // Resolve the farm's server id so the crop can be POSTed even on an already-synced farm.
  ...
  await db.insert(crops).values({
    ...
    expectedHarvestDate: input.expectedHarvestDate ?? suggestHarvestDate(input.plantingDate),
...
/** Convenience: mark a crop harvested with its outcome. */
